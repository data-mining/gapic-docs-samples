...
class ResumeServiceClient(object):
        ...
        if transport:
            if callable(transport):
                self.transport = transport(
                    ...
                    default_class=resume_service_grpc_transport.
                    ResumeServiceGrpcTransport,
                )
            else:
                if credentials:
                    raise ValueError(
    ...
    def parse_resume(self,
        ...
        if 'parse_resume' not in self._inner_api_calls:
            self._inner_api_calls[
                'parse_resume'] = google.api_core.gapic_v1.method.wrap_method(
                    self.transport.parse_resume,
                    default_retry=self._method_configs['ParseResume'].retry,
                    default_timeout=self._method_configs['ParseResume'].
                    timeout,
