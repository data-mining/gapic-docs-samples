...
class CommuteFilter(object):
    class CommuteMethod(enum.IntEnum):
        ...
        Attributes:
          ...
          BICYCLING (int): Commute time is calculated based on biking time.
...
class Company(object):
    class DataLearningType(enum.IntEnum):
        """
        Enum that represents how user data owned by the company is used.

        Attributes:
          DATA_LEARNING_TYPE_UNSPECIFIED (int): Default value.
          AGGREGATED (int): Data owned by this company is used to improve search/recommendation
          quality across companies.
          ISOLATED (int): Data owned by this company is used to improve search/recommendation
